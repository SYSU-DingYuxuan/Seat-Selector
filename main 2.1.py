...
root = Tk()
root.title(f'选座位 {VERSION} by 丁誉轩')
...
def open_file():
    ...
    file_path = filedialog.askopenfilename(title = '打开座位表', filetypes = [('丁誉轩的座位表文件', '.dyxseat')], initialdir = PATH)
...
def helpo():
    ...
    help_text = f'''选座位助手 {VERSION} 使用帮助

    注意！！本软件窗口分辨率 1600x1000 ，过低的分辨率会导致窗口显示不全。建议将屏幕的实际分辨率调整至 1920x1080 以上。

----------------------------------------------------更新日志----------------------------------------------------
1.0 -> 2.0  Update: 2024-07-14
        选座位助手 2.0 相较于之前的 1.0 版本，新增了撤销、跳过、自定义座位、保存、导出为图片等实用且简洁易用的功能。每次操作后，程序都将自动保存当前座位表。详细信息敬请前往 B 站，观看代码编写全过程和视频使用教程！
    
2.0 -> 2.1  Update: 2024-08-08
        修复了 [执行“跳过-选择-撤销”组合操作后出现座位表错误] 的 BUG
        添加文本标签“Command Shell”
-----------------------------------------------------------------------------------------------------------------
...
感谢大家的使用！如有问题欢迎反馈！
    ...
    help_win_opened = 1
    help_win = Tk()
    help_win.geometry('800x570')
    help_win.title('帮助')
    help_win.resizable(False, False)
    help_win.iconbitmap(PATH + '\\Threat.contrast-black.ico')
    help_win.wm_attributes('-topmost', 1)

    TPT = Text(help_win, font = ('微软雅黑', 13))
    TPT.place(x = 0, y = 0, width = 800, height = 570)
...
Label(root, text = 'Command Shell', bg = 'black', fg = 'white', font = ('consolas', 18)).place(x = 1200, y = 0, height = 40, width = 400, anchor = 'nw')
Label(root, text = '___________________________________________' * 5, bg = 'black', fg = 'white', font = ('consolas', 3)).place(x = 1200, y = 35, height = 5, width = 400, anchor = 'nw')
...
history_viewer.place(x = 1200, y = 40, width = 400, height = 560)
